...
class Space(object):
    ...
    def move_player(self,player,board):
        ...
        if self.move > 0:
            ...
            print("this space ha s a chute "+player.peice+"moves down",self.move,"spaces")
        ...
        if self.move > 0 or self.move < 0:
            if player.player_num == 1:
                ...
                p2p[oldpos] = EMPTY
                p2p[player.position] = player.peice
            elif player.player_num == 3:
                p3p[oldpos] = EMPTY
                p3p[player.position] = player.peice
            elif player.player_num == 4:
                p4p[oldpos] = EMPTY
                p4p[player.position] = player.peice
            board.display_board()
